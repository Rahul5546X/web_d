...
function formValidation() {
  ...
  const nameDiv = form.querySelector("#fname");
  const mailDiv = form.querySelector("#e-mail");
  const reviewDiv = form.querySelector("#text-area");
   ...
   */
   ...
   if (name.length <= 0 || mail.length <= 0 || userReview.length <= 0) {
    //  alert("Please Fill All The Fields.");
      warningSpan.innerHTML = `Name is not valid.`
      nameDiv.appendChild(warningSpan);
      // mailDiv.append(warningSpan);
      // reviewDiv.append(warningSpan);
     return [false];
   }
...
function emptyFieldValidator(event) {
  event.preventDefault();
  ...
  if (userName.length === 0) {
      ...
      warningSpan.innerHTML = "Name is required.";
      nameDiv.appendChild(warningSpan);
      valid = false;
  }
  ...
  if (userMail.length === 0) {
      const warningSpan = document.createElement("span");
      warningSpan.className = "warning";
      warningSpan.innerHTML = "Email is required.";
      mailDiv.appendChild(warningSpan);
      valid = false;
  }
  ...
  if (userPassword.length === 0) {
      const warningSpan = document.createElement("span");
      warningSpan.className = "warning";
      warningSpan.innerHTML = "Password is required.";
      passwordDiv.appendChild(warningSpan);
      valid = false;
  }

  function printValues() {
    console.log(`Name is ${userName} and e-mail is ${userMail} and review is "${userPassword}"`);
  }
  if(valid){
    printValues();
  }
            ...
            // const fields = ['userName', 'userMail', 'userPassword'];
            // const fieldsDiv = ['name','mail','password']

            // // Create a single warning message element
            // const warningSpan = document.createElement("span");
            // warningSpan.className = 'warning';
            // warningSpan.textContent = 'This field is required.';

            // // Append warning spans to each input field's parent div
            // fieldsDiv.forEach(field => {
            //     const inputDiv = document.getElementById(`${field}Div`);
            //     const clone = warningSpan.cloneNode(true);
            //     clone.id = `${field}Warning`;
...
document.getElementById("myForm").addEventListener("submit", emptyFieldValidator);
/**
 * getDateTime
...
//     // Ensure the tooltip is within the viewport horizontally
//     const viewportWidth = window.innerWidth;
//     if (tooltipDiv.getBoundingClientRect().left < 0) {
//       tooltipDiv.style.left = '0px';
//     } else if (tooltipDiv.getBoundingClientRect().right > viewportWidth) {
//       tooltipDiv.style.left = `${viewportWidth - tooltipWidth}px`;
//     }
//   }
// }
...
// }
function twoLevelDropdown() {
  const dropDowns = document.querySelectorAll(".has_drop_down");
